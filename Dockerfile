...
# Comment
USER root:root
RUN yum -y install make.x86_64 unzip gcc.x86_64 bison flex readline.x86_64 readline-devel.x86_64 zlib.x86_64 zlib-devel.x86_64 openssl-devel libxml2 libxml2-devel git wget libnsl libaio glibc-langpack-en glibc-common.x86_64 langpacks-en.noarch glibc-locale-source
...
RUN pwd
RUN cp instantclient_19_14/*.so* /usr/lib
...
RUN    mkdir -p /var/lib/pgsql/data && mkdir -p $STI_SCRIPTS_PATH && mkdir -p /var/run/postgresql
...
RUN ln -s /usr/bin/run-postgresql /usr/libexec/s2i/run
